        ...
        if (ping_pkt->type == PING) {
            ...
            if (last_seq_rcv > ping_pkt->seq) {
                ...
                if (setup->useTimedBuffer()) {
                    ...
                    pthread_mutex_lock(setup->getMutex());
                    *output << ss.str();
                    pthread_mutex_unlock(setup->getMutex());
    ...
    for (unsigned int i = 1; i <= setup->getCount(); i++) {
        ...
        if (!setup->nextPacket()) {
            ...
            if (setup->useTimedBuffer() || setup->actWaiting()) {
                ...
                //std::cerr << tgTime << " " << ts.tv_sec << " " << ts.tv_nsec << std::endl;
                setup->recordLastDelay(ts);
                stats->setStatus("before delay");
                delay(ts);
                stats->setStatus("after delay");
                clock_gettime(CLOCK_REALTIME, &curTv);
            }
        } else {
            stop = true;
            std::cout << "stop activated on sender - there is no next packet" << std::endl;
            break;
        }
        stats->setStatus("after pkt timing");
        ...
        if (setup->showSendBitrate()) {
            ...
            stats->setStatus("before to JSON");
            if (setup->toJSON()) {
                ss << "\"direction\":\"tx\",\n\t\t\t\"size\":" << nRet << ",\n\t\t\t\"seq\":" << ping_pkt->seq << ",\n\t\t\t";
                ss.precision(3);
                ss.setf(ios_base::right, ios_base::adjustfield);
            ...
            if (setup->useTimedBuffer()) {
                ...
                stats->setStatus("before fprintf");
                //fprintf(fp, "%s", ss.str().c_str());
                pthread_mutex_lock(setup->getMutex());
                *output << ss.str() << std::flush;
                pthread_mutex_unlock(setup->getMutex());
                stats->setStatus("after fprintf");
            }
            stats->setStatus("at the end of loop");
        }
        stats->setStatus("after show / end of main loop");
